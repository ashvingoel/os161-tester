...
def generate_mail(TA, email, msgtextfile, assignment, files):
    msg = MIMEMultipart()
    msg['From'] = TA
    msg['To'] = email
    msg['Subject'] = 'ECE344: automarker results for assignment ' + assignment
    try:
...
def generate_mbox(TA, email, msgtextfile, assignment, grp):
	...
	mbox.lock()
	try:
            files = ["os161-pexpect-" + grp + ".log",
                     "os161-marker-" + grp + ".log",
                     "os161-tester-" + grp + ".log",
                     "os161-mark-" + grp + ".txt"]
            msg = generate_mail(TA, email, msgtextfile, assignment, files)
            if msg is not None:
                mbox.add(msg)
                mbox.flush()
	finally:
